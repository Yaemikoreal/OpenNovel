...
from loom.storage.yaml_storage import YAMLStorage, ConflictError
...
class TestReadWrite:
    ...
    def test_atomic_write_content_integrity(
        self, storage: YAMLStorage, tmp_path: Path
    ) -> None:
        """测试原子写入后文件内容完整。"""
...
class TestUpdateFrontmatter:
    ...
    def test_update_preserves_body(
        self, storage: YAMLStorage, sample_md_file: Path
    ) -> None:
        """测试更新 Frontmatter 时保持正文不变。"""
        updated = storage.update_frontmatter(
            sample_md_file, {"name": "Alice Updated"}
        )
...
class TestExtractFunctions:
    ...
    def test_get_frontmatter_value(
        ...
        assert value == "Alice"
        assert (
            storage.get_frontmatter_value(sample_md_file, "nonexistent")
            is None
        )
