    ...
    def _apply_updates(self, result: ManagerUpdateResult) -> list[str]:
        ...
        for update in result.character_updates:
            try:
                ...
                logger.info("角色更新: %s.%s = %s (%s)",
                            update.character_id, update.field, update.value, update.reason)
            except Exception as e:
                logger.error("角色更新失败 %s.%s: %s",
                             update.character_id, update.field, e)
        ...
        for attempt in range(MAX_RETRIES + 1):
            try:
                ...
                logger.info(
                    ...
                    chapter_id, len(result.character_updates), len(event_ids),
                )
                return result
            except (json.JSONDecodeError, ValueError, KeyError) as e:
                last_error = e
                logger.warning("Manager 提取 JSON 解析失败 (尝试 %d/%d): %s",
