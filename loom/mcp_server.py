"""LOOM MCP Server - 通过 MCP 协议暴露 LOOM 创作能力。
...
from loom.core.auto_runner import AutoRunner
...
async def list_tools() -> list[types.Tool]:
    """返回 LOOM MCP Server 暴露的所有 tools。"""
    return [
        types.Tool(
            name="init_project",
            description=(
                "初始化 LOOM 小说项目。创建标准目录结构（canon/characters/draft/"
...
async def _handle_init_project(args: dict) -> str:
    ...
    if not canon_path.exists():
        storage.write_markdown_file(
            canon_path, canon_template, "# 世界观设定\n\n在此书写不可违反的世界观规则...",
        )
    ...
    if not config_path.exists():
        ...
        results.append("✓ 创建配置文件: loom.yaml")
    ...
    if draft_dir.exists():
        ...
        for cf in ch_files:
            try:
                ...
                word_count = len(body)
                sections.append(f"- {meta.get('id', cf.stem)}: {meta.get('title', '无标题')} ({word_count} 字)")
            except Exception:
                sections.append(f"- {cf.stem}: (读取失败)")
